...
class SpeechTrainer(BaseTrain):
    ...
    def train_step(self):
        ...
        loss, _, decoded, error = self.session.run([
            ...
            self.model.dropout_placeholder: self.config.dropout_prob(),
            self.handle_placeholder: self.test_handle
        })
    ...
    def prepare_dataset(self):
        ...
        iter = tf.data.Iterator.from_string_handle(
            self.handle_placeholder, train_dataset.output_types, train_dataset.output_shapes, train_dataset.output_classes)
        # iter.get_next() #???
        #input, sparse_label, seq_length = train_iterator.get_next()
        input, sparse_label, seq_length = iter.get_next()
        inputs = {
            'input': input,
